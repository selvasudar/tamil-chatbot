...
if uploaded_file is not None:
    ...
    if st.session_state.get("vectorstore") and st.sidebar.checkbox("🔧 Debug Mode"):
        ...
        if st.sidebar.button("Test Retrieval"):
            test_query = st.sidebar.text_input("Test Query", value="test")
            if test_query:
                retriever = st.session_state.vectorstore.as_retriever(search_kwargs={"k": 3})
                docs = retriever.get_relevant_documents(test_query)
                st.sidebar.write(f"Retrieved {len(docs)} documents for query: '{test_query}'")
        ...
        try:
            ...
            st.sidebar.write(f"Document count: {collection.count()}")
        except Exception as e:
            st.sidebar.error(f"Error getting collection stats: {e}")
...
### 📦 Required Packages:
```bash
pip install streamlit PyPDF2 pdfplumber langchain langchain-google-genai langchain-huggingface chromadb sentence-transformers python-dotenv
